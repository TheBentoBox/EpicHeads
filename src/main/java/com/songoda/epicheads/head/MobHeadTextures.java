...
public enum MobHeadTextures {
    ...
    SNOW_GOLEM("a528df1653962e4e995dfd064a72b2cbff9b7197912880143f941a37db46c"),
    ...
    private static final Map<String, MobHeadTextures> BY_TYPE = new HashMap<>();

    static {
        for (MobHeadTextures texture : values()) {
            BY_TYPE.put(texture.name(), texture);
        }
        BY_TYPE.put("MUSHROOM_COW", MOOSHROOM);
        BY_TYPE.put("SNOWMAN", SNOW_GOLEM);
        BY_TYPE.put("PIG_ZOMBIE", ZOMBIFIED_PIGLIN);
    }

    private final String urlHash;

    MobHeadTextures(String urlHash) {
    ...
    public static Optional<MobHeadTextures> from(EntityType type) {
        if (type == null) {
            return Optional.empty();
        }
        return Optional.ofNullable(BY_TYPE.get(type.name()));
    }
}
